...
describe('GET /api', () => {
    ...
    each(correctBrand).test('Schuh, der die Marke %s hat',
        async (brand: string) => {
            
            const params = { marke: brand };

            const response: AxiosResponse<SchuheVO> = await client.get(apiPath,{ params },);
            const { status, headers, data } = response;
            ...
            schuhe
                ...
                .forEach((marke: string) => 
                    expect(marke).toEqual(brand),
                );
        },
    );

    each(wrongBrand).test('Kein Schuh, der die Marke %s hat',
        async (brand: string) => {
